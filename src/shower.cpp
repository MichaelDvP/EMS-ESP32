...
void Shower::send_mqtt_stat(bool state, bool forceHA) {
    ...
    if ((Mqtt::ha_enabled()) && (!ha_configdone_ || forceHA)) {
        ha_configdone_ = true;
        ...
        doc.clear();
        doc["name"]        = FJSON("Shower Duration");
        doc["uniq_id"]     = FJSON("shower_duration");
        doc["~"]           = Mqtt::base();
