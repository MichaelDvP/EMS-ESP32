...
void DallasSensor::loop() {
    ...
    if (state_ == State::IDLE) {
        ...
        if (temperature_convert_complete() && (time_now - last_activity_ > CONVERSION_MS)) {
            ...
            LOG_WARNING(F("Sensor read timeout"));
            state_ = State::IDLE;
            sensorfails_++;
        }
...
std::string DallasSensor::get_name(const char * id) {
    std::string name = id;

    EMSESP::webSettingsService.read([&](WebSettings & settings) {
        for (uint8_t i = 0; i < NUM_SENSOR_NAMES; i++) {
            if (strcmp(settings.sensor[i].id.c_str(), id)  == 0) {
                name = settings.sensor[i].name.c_str();
                name = name + " : ";
                char buf[10];
                name = name + Helpers::render_value(buf, settings.sensor[i].offset, 10);
            }
        }
    });
...
void DallasSensor::publish_sensor(Sensor sensor) {
   if (Mqtt::subscribe_format() == Mqtt::Subscribe_Format::GENERAL) {
       return;
   }
    uint8_t  no = 0;
    while (no < sensors_.size() && sensors_[no].id() != sensor.id()) {
        no++;
    }
    char topic[Mqtt::MQTT_TOPIC_MAX_SIZE];
    if (dallas_format_ == Dallas_Format::NUMBER) {
        ...
        snprintf_P(topic, sizeof(topic), PSTR("%s/%s"), uuid::read_flash_string(F_(sensor)).c_str() ,sensor.to_string().c_str());
    }
    char payload[10];
    EMSESP::webSettingsService.read([&](WebSettings & settings) {
    ...
    for (const auto & sensor : sensors_) {
        ...
        if (Mqtt::ha_enabled()) {
            if (!(registered_ha_[sensor_no - 1]) || force) {
                ...
                if (dallas_format_ == Dallas_Format::NUMBER) {
                    snprintf_P(str, sizeof(str), PSTR("Sensor %d"), sensor_no);
                } else {
                ...
                snprintf_P(str, sizeof(str), PSTR("sensor_%s"), sensor.to_string().c_str());
                config["uniq_id"] = str;

                JsonObject dev = config.createNestedObject("dev");
                dev["name"]    = FJSON("EMS-ESP Sensor");               // Global name for device (all Dallas sensors, avoids using the very first name for the group)
